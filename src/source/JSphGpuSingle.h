...
public:
  ...
  /// Library mode: initialise only (no main loop).
  void RunInit(std::string appname,const JSphCfgRun* cfg,JLog2* log);
  /// Library mode: advance one timestep.  Returns the step dt.
  double RunStep();
  /// Library mode: finalise the simulation (end-of-run summary, cleanup).
  void RunFinish();

  // ── Public accessors for library mode ─────────────────────────────────
  double      GetTimeStep()  const{ return(TimeStep); }
  double      GetTimeMax()   const{ return(TimeMax);  }
  unsigned    GetNp()        const{ return(Np);       }
  unsigned    GetNpb()       const{ return(Npb);      }

  const double2*  GetPosxyPtr()  const{ return(Posxy_g  ? Posxy_g->cptr()  : NULL); }
  const double*   GetPoszPtr()   const{ return(Posz_g   ? Posz_g->cptr()   : NULL); }
  const float4*   GetVelrhoPtr() const{ return(Velrho_g ? Velrho_g->cptr() : NULL); }
  const unsigned* GetIdpPtr()    const{ return(Idp_g    ? Idp_g->cptr()    : NULL); }
  const typecode* GetCodePtr()   const{ return(Code_g   ? Code_g->cptr()   : NULL); }
